...
/**
 * Render status orders
 *
 * @param array  $orders Orders array.
 * @param string $status Order status.
 * @return string
 */
function anony_render_status_orders( $orders, $status ) {
 ...
 *
 * @param array  $statuses An array of order status.
 * @param string $has_orders If user has orders.
 * @param int    $current_page Current page.
 * @return void
 */
function anony_app_render_orders( $statuses, $has_orders, $current_page = 1 ) {
	$current_page    = empty( $current_page ) ? 1 : absint( $current_page );
	$wp_button_class = wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '';
	$anony_options   = ANONY_Options_Model::get_instance();

	$orders_query_args = apply_filters(
		'woocommerce_my_account_my_orders_query',
		array(
			'status'         => array_keys( $statuses ),
			'page'           => $current_page,
			'posts_per_page' => ! empty( $anony_options->orders_per_page ) ? $anony_options->orders_per_page : 20,
			'paginate'       => true,
			'customer_id'    => get_current_user_id(),
		)
	);

	$customer_orders = wc_get_orders( $orders_query_args );
	?>
	<style>
			.order-container {
	...
	if ( $customer_orders && ! empty( $customer_orders ) ) {
		...
		$all_orders = array();
		foreach ( $orders as $order ) {
			$all_orders[ 'wc-' . $order->get_status() ][] = $order;
		}

		$tab_items = '';
		$content   = '';

		foreach ( $all_orders as $status => $status_orders ) {
			$label      = trim( $statuses[ $status ], '()' );
			...
			//phpcs:disable
			$content .= '<div class="order-tab-content' . $active . '" id="' . esc_attr( $status ) . '-tab">';
			//phpcs:enable
			$content .= anony_render_status_orders( $status_orders, $status );
			$content .= '</div>';
		}
		...
		$output = $tabs . '<div id="anony-orders-content" class="anony-grid-row anony-grid-col">' . $content . '</div>';
...
function anony_custom_app_orders( $atts ) {
	...
	ob_start();
	anony_app_render_orders( $statuses, $has_orders, $atts['current_page'] );
	return ob_get_clean();
}
add_action(
	'wp_footer',
