...
*/
		...
		--------------------------------------------------------------------------
			 ...
			 *-------------------------------------------------------------------------
*/
			if ( isset( $this->user_data['user_pass'] ) ) {
				$this->set_user_pass( $this->user_data['user_pass'] );
			}
		}
		...
		public function force_user_login() {
			if ( $this->email_exists ) {
				...
				if ( $this->_user->user_login !== $user_login ) {
					...
					$user_id = $this->set_user_login( $user_data, array( 'ID' => $this->_user->ID ) );
		...
		public function set_user_login( $user_data, $id = null ) {
			...
			if ( ! isset( $id['ID'] ) || ! is_integer( $id['ID'] ) ) {
				return;
			}
		...
		public function set_user_pass( $password ) {
			...
			if ( $this->_user->user_pass !== md5( $password ) ) {
				...
				if ( $this->_wpdb->update( $this->_wpdb->users, $user_data, array( 'ID' => $this->_user->ID ) ) ) {
					$this->crids_updated = true;
				}
			}
		}
